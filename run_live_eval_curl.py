"""
LLM-TOP Live API Empirical Evaluation via Native System Curl Engine
Model: meta/llama-3.3-70b-instruct (NVIDIA NIM Endpoint)
...
# Fetch token safely from process or user environment
...
# Generic, privacy-compliant tasks
TASKS = [
    {
...
def query_nvidia_curl(system_prompt: str, user_prompt: str) -> str:
    payload = {
        "model": MODEL_NAME,
        "messages": [
    ...
    cmd = [
        "curl.exe", "-s", "-m", "120", "-X", "POST", NVIDIA_URL,
    ...
    if res.returncode != 0:
        raise RuntimeError(f"Curl failed with return code {res.returncode}: {res.stderr}")
    ...
    if "choices" in data and len(data["choices"]) > 0:
        ...
        raise RuntimeError(f"Unexpected response format: {res.stdout[:200]}")
...
def run_eval():
    ...
    for idx, t in enumerate(TASKS, 1):
        ...
        print(f"[{idx}/{len(TASKS)}] Querying Llama 3.3 70B for '{t['name']}'...")

        # 1. LLM-TOP Call
        start_t = time.time()
        llmtop_raw = query_nvidia_curl(SYS_LLMTOP, user_msg)
        llmtop_time = time.time() - start_t
        clean_llmtop = re.sub(r'```[a-zA-Z]*', '', llmtop_raw).strip('` \n\r')

        # 2. Minified JSON Call
        start_t = time.time()
        json_raw = query_nvidia_curl(SYS_JSON, user_msg)
